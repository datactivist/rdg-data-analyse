            ...
            for lien in liens_group.find_all("a"):
                if "dataverse" in lien['href'] :
                    ...
                    entry = {
                        ...
                        "Nombre de collections supposé" : 0 ,
...
def extraire_nbr_datasets(html_content):
    ...
    if span and span.text.strip():
        match = re.search(r'Datasets \(([\d,.]+)\)', span.text.strip())  # Capture 3,106 ou 2.500
        if match:
            nombre_jdd = match.group(1).replace(",", "").replace(".", "")  # Supprime les séparateurs
...
for entry in all_data:
    ...
    total_collections = 0
    list_identifier = entry.get("identifier", [])   

    while True :
        lien_complet = f"{lien}&page={num_page}" if "?" in lien else f"{lien}?page={num_page}"
        html_filename = html_dir / f"{entry['Espace institutionnel'].replace(' ', '_')}_{num_page}.html"
        if html_filename.exists():
            ...
            if rep.status_code != 200:
                print(f"Erreur lors de la récupération de la page {num_page} pour {lien}" )
        ...
        if not nombre_jdd_extrait:
            ...
            print(f"{entry['Espace institutionnel']} - {entry['Nombre de jeux de données']} jeux de données")
        ...
        if nbr_collections == 0:
            ...
            break
        entry["Nombre de collections supposé"] = nbr_collections

        dataverse_html = BeautifulSoup(html_text, "html.parser")
        # Trouver les sous-collections

        table = dataverse_html.find("table", {"id": "resultsTable"})
        if not table:
            print("Aucun tableau trouvé")  
            break
        tds = table.find_all("td")

        sous_collections = dataverse_html.find_all("div", class_="dataverseResult clearfix")
        for collection in sous_collections :
            ...
            lien_sous_collection = collection.find("a")["href"] if collection.find("a") else ""
            if lien_sous_collection:
                sub_identifier = extract_identifier_from_href(lien_sous_collection)
                if sub_identifier not in list_id :
                    list_id.append(sub_identifier)
            ...
            total_collections += len(collection)
            entry["Nombre total de sous-collections "] = total_collections

        next_page = dataverse_html.find("a", text=re.compile(r"Suivant|Next"))
        if not next_page:
            break
...
for entry in all_data:
    ...
    nbr_collections = entry["Nombre de collections supposé"]
    total_collections = len(entry["Sous-collections"])
    sous_collections = ", ".join(entry["Sous-collections"])  # Transformer la liste en une chaîne
    nbr_jdds =entry["Nombre de jeux de données"]
    list_identifier = entry["identifier"]
...
df = pd.DataFrame([(key, value[0], value[1] , value[2] , value[3]) for key, value in data_dict.items()],  
                  columns=["Espace institutionnel" ,"Nombre de sous-collections" , "Nombre de jeux de données", "Sous-collections" , "identifier"])


with pd.ExcelWriter("espaces_institutionnels_collections.xlsx", engine="openpyxl") as writer:
...
for entry in all_data:
    ...
    for sous_collection in entry["Sous-collections"]:
        expanded_data.append({
            ...
            "Sous-collection": sous_collection 
        })
...
with pd.ExcelWriter(output_excel_file_2, engine="openpyxl") as writer:
    for espace, group in df_expanded.groupby("Espace institutionnel"):
        ...
        group[["Sous-collection"]].to_excel(writer, sheet_name=sheet_name, index=False)  # Supprimer la colonne inutile


output_csv_file = Path("espaces_institutionnels_identifiers.csv")
...
with open("sous_collections_sans_doublon", mode='w', newline='', encoding='utf-8') as f:
    writer = csv.writer(f)
    writer.writerow(["collection"])  # En-tête
    for collection in list_id:
